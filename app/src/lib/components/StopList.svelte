<script lang="ts">
	...
	function stopIcon(i: number, n: number): string {
		if (i === 0) return '<span class="marker"><span class="origin-dot"></span></span>';
		if (i === n - 1)
			return '<span class="marker"><svg class="dest-pin" viewBox="0 0 24 24"><path fill="#ff5a52" d="M12 2C7.86 2 4.5 5.36 4.5 9.5c0 5.62 6.5 12 7.09 12.56a.55.55 0 00.82 0C13 21.5 19.5 15.12 19.5 9.5 19.5 5.36 16.14 2 12 2zm0 10.25A2.75 2.75 0 1112 6.75a2.75 2.75 0 010 5.5z"/></svg></span>';
		return '<span class="marker"><span class="waypoint-dot"></span></span>';
	}

	function stopPlaceholder(i: number, n: number): string {
		if (i === 0) return 'Choose starting point';
		if (i === n - 1) return 'Choose destination';
		return `Add stop ${i}`;
	}

	async function positionRail() {
		await tick();
	...
	$effect(() => {
		...
		app.stops.length; // eslint-disable-line @typescript-eslint/no-unused-expressions
		positionRail();
	});
	...
	function onInput(i: number, e: Event) {
		...
		app.setStopValue(i, value);
		scheduleAutocomplete(i, value);
	}

	function onFocus(i: number, e: Event) {
		app.focusStop(i);
...
<div class="searchbox" bind:this={searchBoxEl}>
	<div class="rail" bind:this={railEl}></div>
	<div bind:this={rowsEl}>
		{#each app.stops as s, i (app.stops.length + '-' + i)}
			<div class="searchrow">
				{@html stopIcon(i, app.stops.length)}
				<input
					type="text"
					class="stopInput"
					placeholder={stopPlaceholder(i, app.stops.length)}
					autocomplete="off"
					required
					role="combobox"
					aria-expanded={app.acIdx === i && app.acResults.length > 0}
				...
				/>
				<button type="button" class="rowbtn save" aria-label="Save this place" title="Save this place" onclick={() => savePreset(i)}>
					<svg viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
				</button>
				{#if i > 0 && i < app.stops.length - 1}
					<button type="button" class="rowbtn remove" aria-label="Remove this stop" title="Remove this stop" onclick={() => removeStop(i)}>
	...
	<button type="button" class="swapbtn" aria-label="Reverse stop order" title="Reverse stop order" onclick={() => app.reverseStops()}>
	...
	{#if app.acIdx !== null && (app.acResults.length > 0 || app.acResults.length === 0 && app.acActive === -2)}
		<div class="ac-list" role="listbox" style="position:fixed;">
			{#if app.acResults.length === 0}
				<div class="ac-empty">No matches nearby</div>
			{:else}
				{#each app.acResults as r, i (r.lat + ',' + r.lon)}
					<div class="ac-item" class:active={app.acActive === i} role="option" onmousedown={(e) => { e.preventDefault(); pickResult(i); }}>
