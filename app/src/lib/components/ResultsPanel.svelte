<script lang="ts">
	...
	import { CheckCircle, AlertTriangle, XCircle, Play, Square } from '$lib/icons/index.js';
...
{#if domain.results}
	<div class="results">
		<div class="alert {verdictCls} items-start shadow-none">
			{#if domain.results.verdict.level === 'ok'}
				<CheckCircle class="mt-0.5 size-5 shrink-0" />
			{:else if domain.results.verdict.level === 'caution'}
		...
		{#if !session.planning}
			<div class="mt-3 grid grid-cols-2 gap-2">
				<div class="stat animation-rise card-elevated rounded-2xl py-3">
		...
		<div class="mt-3">
			<button
				...
				title={session.trackingActive ? 'Stop GPS tracking' : 'Follow this planned route with live GPS'}
			>
			...
			{#if session.trackingStats}
				<div class="mt-2 grid grid-cols-3 gap-2">
					<div class="stat rounded-xl stat-tile px-3 py-2">
